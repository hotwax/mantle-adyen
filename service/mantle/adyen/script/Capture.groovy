...
// paymentRefNum is pspReference transaction id and we can use it to seattle the payment
try {
    ...
    captureRequest.merchantAccount(paymentGatewayInfo.pgcInfoMap.merchantAccount).originalReference(paymentRefNum).reference(payment.paymentId);
    ...
    captureRequest.setModificationAmount(modificationAmount);
    ...
    if("[capture-received]".equals(response)) {
        Map createPgrOut = ec.service.sync().name("create#mantle.account.method.PaymentGatewayResponse").parameters([
                ...
                paymentMethodId:payment.paymentMethodId, amountUomId:payment.amountUomId, amount:payAmount,
